...
//==================================================================================
    ...
    //Volumes - ca in tally f15:p 5.05 0 23.15 1.7 + e15 0 1e-6 150i 0.15 din MCNP

    G4Orb *solidUnion = new G4Orb("solidUnion", 1.7*cm);  //detectorul sferic folosit in MCNP la tally f5:p 100 0 23.15 10 (x,y,z,r)
    G4LogicalVolume *logicUnion = new G4LogicalVolume(solidUnion, worldMat, "logicUnion");  //volum de detectie cu aer
    G4VPhysicalVolume *physUnion = new G4PVPlacement(0, G4ThreeVector(5.05*cm, 0, 23.15*cm), logicUnion, "physUnion", logicWorld, false, 0, true);
...
//==================================================================================
    ...
    logicUnion->SetVisAttributes(G4Colour::Brown());
...
void MyDetectorConstruction::ConstructSDandField()
{
    ...
    G4MultiFunctionalDetector *energyDet;
    G4SDParticleFilter *gammaFilter;

    energyDet = new G4MultiFunctionalDetector("energyDet");
    G4SDManager::GetSDMpointer()->AddNewDetector(energyDet);

    primitive = new G4PSEnergyDeposit("Edep");  //energia depozitata

    gammaFilter = new G4SDParticleFilter("gammaFilter", "gamma");  //se inregistreaza doar particulele gamma
    gammaFilter->add("gamma");    
    primitive->SetFilter(gammaFilter);

    energyDet->RegisterPrimitive(primitive);  //inregistrarea primitivei "primitive" in "energyDet"
    
    SetSensitiveDetector("logicUnion", energyDet);  //volumului logic "logicUnion" i s-a atribuit variabila "energyDet"
}