...
int main(int argc, char** argv)
{
    ...
    unsigned long long int totalEvents = 1;  //Aici se modifica nr. total de evenimente dorit a fi simulat!!!
    G4bool visualization = true;  //Se seteaza: true = cu vizualizare, false = fara vizualizare
    G4int visEvents = 15;  //Aici se modifica nr. de evenimente care se doreste a fi vazut cu GUI!!!
    ...
    if(!visualization)
    {
        for(G4int i=0; i<nOfRuns; ++i)
        {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(1500));  //time-delay intre Run-uri pt. a ii da sistemului timp sa elibereze memoria
        }
